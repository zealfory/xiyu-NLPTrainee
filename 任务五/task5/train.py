...
from net import NET_RNN, DatasetRNN
...
parser.add_argument('--batchsize', default=config.BATCH_SIZE, help='default batchsize is 1')
...
def train(method='RNN'):
    ...
    if os.path.exists('%s/data/x_seq.pkl' % DIR):
        ...
        texts = processdata.load_data(len_min=20, len_max=200)
        data_process = processdata.text2seq(texts=texts, num_words=None, maxlen=200)
    ...
    if method == 'RNN':
        ...
        trainloader = torch.utils.data.DataLoader(
            dataset=DatasetRNN(x_seq[:-100], y_seq[:-100]),
